...
- **Remaining real-stack work (still worth running here):** the full §V 5-site matrix as a REPL scenario; the other lodestar probes (`probe_live_concept_graph`, `probe_live_agent`, `probe_live_iterated_compile`, `probe_live_scan_with_cleanup`); Phase 2/3 live-BROWSER render verification (caret/IME, halo paint).
