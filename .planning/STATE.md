...
- **Still deferred to the GPU box (dynamic DOM only):** the §V 5-site LIVE matrix + `all_real` probes add value mainly for JS-rendered content the saved static HTML misses + the real retrieval/halo render; the static-DOM extraction→content-tree path is now offline-verified across the corpus.
