# Test Generation Task: tastedive_commovieslikeAmiciMieiAttoIiideg.html
...
**Structural chunks:**
  - Chunk 0: `a.ab.details` (freq=46, pq_sig=`div(meta,meta,meta,div(div(a(div(div(img)),div(div`)
  - Chunk 1: `button.button.jq` (freq=834, pq_sig=`button(div(svg(path)),span,span)`)
  - Chunk 2: `div.sc.slrv` (freq=17, pq_sig=`div(div(img))`)

**Text/Nav chunks:**
  - Chunk 5: `[text_content:p×2]` (freq=1)
  - Chunk 6: `[text_content:p×4]` (freq=4)
  ...
  - Chunk 22: `[nav_content:a×137]` (freq=137)
...
```
=== QUALITY REPORT: tastedive_commovieslikeAmiciMieiAttoIiideg.html ===
Chunks: 175 (structural=3, functional=4, text=168)
Categories: {'card': 2, 'button': 1, 'search_input': 1, 'pagination': 1, 'text_singleton': 168, 'menu_item': 2}
Content: 215 tagged, 215 preserved (100%)
Leaks: 0 total, 0 high-importance

--- Chunk 0: a.ab.details ---
  type=card freq=46 excl=0.00
  pattern_xpath: ///a
...
--- Chunk 1: button.button.jq ---
  type=button freq=834 excl=1.00
...
--- Chunk 22: [nav_content:a×137] ---
  type=menu_item freq=137 excl=0.00
  pattern_xpath: ///a
...
4. Use filename `tastedive_commovieslikeAmiciMieiAttoIiideg.html` in all `_distill()` calls
5. Name the module `test_tastedive_commovieslikeAmiciMieiAttoIiideg_gen.py`
