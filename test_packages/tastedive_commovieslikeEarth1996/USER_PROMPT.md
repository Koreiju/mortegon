# Test Generation Task: tastedive_commovieslikeEarth1996.html
...
**Structural chunks:**
  - Chunk 0: `a.details.entity` (freq=50, pq_sig=`div(meta,meta,meta,div(div(a(div(div(img)),div(div`)
  - Chunk 1: `button.button.jq` (freq=852, pq_sig=`button(div(svg(path)),span,span)`)
  - Chunk 2: `div.sc.slrv` (freq=20, pq_sig=`div(div(img))`)
...
```
=== QUALITY REPORT: tastedive_commovieslikeEarth1996.html ===
Chunks: 186 (structural=3, functional=4, text=179)
Categories: {'card': 2, 'button': 1, 'search_input': 1, 'pagination': 1, 'text_singleton': 179, 'menu_item': 2}
Content: 226 tagged, 226 preserved (100%)
...
--- Chunk 1: button.button.jq ---
  type=button freq=852 excl=0.90
...
--- Chunk 4: [pagination_buttons] ---
  type=pagination freq=50 excl=0.00
...
4. Use filename `tastedive_commovieslikeEarth1996.html` in all `_distill()` calls
5. Name the module `test_tastedive_commovieslikeEarth1996_gen.py`
