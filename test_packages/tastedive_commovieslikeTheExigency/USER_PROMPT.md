# Test Generation Task: tastedive_commovieslikeTheExigency.html
...
**Structural chunks:**
  - Chunk 0: `a.details.entity` (freq=61, pq_sig=`div(meta,meta,meta,div(div(a(div(div(img)),div(div`)
  - Chunk 1: `button.button.jq` (freq=876, pq_sig=`button(div(svg(path)),span,span)`)
  - Chunk 2: `div.sc.slrv` (freq=26, pq_sig=`div(div(img))`)
  - Chunk 3: `div.sc.slrv` (freq=9, pq_sig=`div(div(div(img)))`)
...
**Text/Nav chunks:**
  ...
  - Chunk 17: `[nav_content:a×145]` (freq=145)
  - Chunk 18: `[text_content:h2×1]` (freq=1)
  - Chunk 19: `[text_content:h3×1]` (freq=1)
...
```
=== QUALITY REPORT: tastedive_commovieslikeTheExigency.html ===
Chunks: 179 (structural=4, functional=4, text=171)
Categories: {'card': 3, 'button': 1, 'search_input': 1, 'pagination': 1, 'text_singleton': 171, 'menu_item': 2}
Content: 221 tagged, 221 preserved (100%)
Leaks: 0 total, 0 high-importance

--- Chunk 0: a.details.entity ---
  type=card freq=61 excl=0.00
  pattern_xpath: ///a
...
--- Chunk 5: [pagination_buttons] ---
  type=pagination freq=53 excl=0.00
...
4. Use filename `tastedive_commovieslikeTheExigency.html` in all `_distill()` calls
5. Name the module `test_tastedive_commovieslikeTheExigency_gen.py`
