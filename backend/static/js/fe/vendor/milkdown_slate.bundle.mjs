...
function getMarkdown(range) {
  return (ctx) => {
    ...
    if (!range) return serializer2(view.state.doc);
    const slice = view.state.doc.slice(range.from, range.to, true);
    const doc4 = schema3.topNodeType.createAndFill(null, slice.content);
    if (!doc4) {
      console.error("No document found");
      return "";
    }
    return serializer2(doc4);
  };
}
...
async function mountMilkdown(host, source, opts = {}) {
  ...
  function setText(text4) {
    suppress = true;
    try {
      editor.action(replaceAll(String(text4 == null ? "" : text4)));
    } finally {
      suppress = false;
    }
  }
