...
def _extraction_trie_to_accessor_map(trie: Dict[str, Any]) -> Dict[str, str]:
    """Flatten a chunk's ``extraction_trie`` into ``{field_name: rel_xpath}``.

    §5.4 — the accessor table for an ``XPathPattern`` concept node. The
    extraction_trie's intermediate keys start with ``/`` (possibly
    compressed multi-tag, e.g. ``"/a/h3"``); terminal keys are data
    addresses (``"text()"`` or ``"@attr"``). The relative xpath for a
    leaf is the concatenation of intermediate keys + ``/`` + leaf key,
    matching the result keys emitted by ``chunk_query._evaluate_trie``
    so downstream consumers can cross-reference the two sources.

    Field names are slugs of the last tag segment + data address tail
    (``/a/@href`` → ``"a_href"``, ``/a/h3/text()`` → ``"h3_text"``). Slug
    collisions disambiguate with a numeric suffix so the map is always a
    bijection.
    """
    if not trie:
        return {}

    leaves: List[Tuple[str, str]] = []  # (intermediate_path, addr)

    def _walk(node: Dict[str, Any], rel_path: str) -> None:
        for k, v in node.items():
            if not isinstance(k, str):
                continue
            if k.startswith("@") or k == "text()":
                leaves.append((rel_path, k))
    ...
    result: Dict[str, str] = {}
    used: Set[str] = set()
    for rel, addr in leaves:
        rel_xpath = f"{rel}/{addr}" if rel else f"/{addr}"
        last_segs = [s for s in rel.split("/") if s]
        last_tag = last_segs[-1].split("[")[0] if last_segs else "root"
        if addr.startswith("@"):
            slug_tail = addr[1:]
        elif addr == "text()":
            slug_tail = "text"
    ...
    *,
    ...
    if persist:
        try:
            ...
            saved = True
            # W11c / §8D.39.3 — auto-materialise XPathPattern concept
            # nodes from the freshly-persisted TriePattern rows. The
            # concept node is the user/agent-facing handle on the
            # pattern's instance bank and accessor mapping.
            try:
                ...
                ge = GraphEditor(db_conn=_c)
                ci = get_concept_index_service(graph_editor=ge)
                mat = CompiledFromScansMaterialiser(graph_editor=ge, concept_index=ci)
                domain = ""
                try:
                    domain = urlparse(getattr(built, 'url', '') or '').hostname or ""
                except Exception:
                    domain = ""
                # Iterate over the BuiltTrie's patterns. Different
                # builds expose patterns under different attribute
                # names; try a few.
                patterns = (
                    getattr(built, 'patterns', None)
                    or getattr(built, 'pattern_rows', None)
                    or []
                )
                # §5.4 — derive per-pattern accessor_map from the chunks'
                # extraction_trie so each XPathPattern concept node carries
                # a usable field selector table at materialisation time
                # (instead of an empty dict that consumers later have to
                # backfill out-of-band). When two chunks share a pattern we
                # keep the richer map — extraction tries are unioned across
                # member subtrees, so a larger map is always a superset.
                pattern_accessor_maps: Dict[str, Dict[str, str]] = {}
                for ch in chunks:
                    acc = _extraction_trie_to_accessor_map(
                        getattr(ch, 'extraction_trie', {}) or {}
                    )
                    if not acc:
                        continue
                    existing = pattern_accessor_maps.get(ch.pattern)
                    if existing is None or len(acc) > len(existing):
                        pattern_accessor_maps[ch.pattern] = acc
                for prow in patterns or []:
                    pat = (
                        getattr(prow, 'pattern', None)
                        or (prow.get('pattern') if isinstance(prow, dict) else None)
                    )
                    if not pat:
                        continue
                    mat.materialise_xpath_pattern(
                        domain=domain or "",
                        pattern=str(pat),
                        accessor_map=pattern_accessor_maps.get(str(pat), {}),
                        instance_count=int(
                            getattr(prow, 'commutation_count', 0)
                            or (prow.get('commutation_count') if isinstance(prow, dict) else 0)
                            or 0
                        ),
                        workspace_id="",
                    )

                # §15.8.2 — build + upsert the live ``pattern_map`` ConceptNode
                # from the freshly-built chunks (golden-trio gated). The merge
                # inside update_pattern_map makes it accrete across snapshots /
                # URLs so the panel grows during the scan (§18.29). Its own
                # try/except so a pattern_map hiccup never blocks the
                # XPathPattern peers above.
                try:
                    from backend.mapper.chunk_builder import build_pattern_schemas
                    url_root = getattr(built, "url", "") or url or ""
                    schemas = build_pattern_schemas(
                        chunks, url_root=url_root, domain=domain or "",
                    )
                    # §3.1 / §18.29 — emit PER DETECTED PATTERN (not one bulk
                    # write) so the pattern_map ConceptNode accretes one
                    # top-level pattern at a time and each detection is a
                    # separately-visible mutation (the user watches patterns
                    # appear during the scan). The merge inside
                    # update_pattern_map makes each call additive.
                    for _phash, _schema in (schemas or {}).items():
                        mat.update_pattern_map(
                            schemas={_phash: _schema},
                            url_root=url_root,
                            domain=domain or "",
                            workspace_id="",
                        )
                except Exception:
                    logger.exception("Pipeline: pattern_map update skipped")
            except Exception:
                # Materialisation is best-effort; trie persistence
                # above is the source of truth.
                logger.exception("Pipeline: XPathPattern materialisation skipped")
        except Exception:
...
def run_pipeline_live(
    ...
    *,
    ...
    ``single_pass=True`` consumes only the first yield of
    ``ShadowDOMScanner.scan`` (no scroll loop) which is faster and
    sufficient for pages that fit above the fold. The default drives
    the full scroll+merge loop, draining every yield until the master
    tree stabilises.
    ...
    # scan() is a generator yielding (master_tree, added_nodes,
    # pre_chunks) — drain it for the multi-pass case, break after the
    # first yield for single-pass. There was historically a separate
    # ``scan_single`` helper, but it no longer exists; the generator's
    # first emission is the equivalent initial snapshot.
    last_tree: Any = None
    for master_tree, _added, _pre_chunks in scanner.scan(
        url,
        max_duration=0 if single_pass else max_duration,
        pause=0.0 if single_pass else pause,
    ):
